...
const resources = [
    {
        ...
        details: [
            ...
            { icon: "account_balance_wallet", text: "Grant: Up to ₹50 Lakhs", color: "text-slate-400" },
            { icon: "event", text: "Deadline: 15 Oct 2024", color: "text-red-600" },
        ],
        action: "Apply Now →", actionStyle: "bg-primary text-slate-100 hover:bg-primary-dark hover:text-white",
        actionLink: "/register",
    },
    {
        ...
        action: "Preview", actionStyle: "border border-white/10 text-slate-300 hover:bg-[#0a0a0a]",
    ...
    },
    {
        ...
        details: [
            { icon: "business", text: "Agency: QCI India", color: "text-slate-400" },
            { icon: "schedule", text: "Process: 4-6 Weeks", color: "text-amber-600" },
        ],
        action: "View Requirements", actionStyle: "border border-white/10 text-slate-300 hover:bg-[#0a0a0a]",
    },
    {
        ...
        details: [
            { icon: "location_on", text: "Location: RIS, New Delhi", color: "text-slate-400" },
            { icon: "groups", text: "Cohorts: Summer 2024", color: "text-slate-400" },
        ],
        action: "Apply Now →", actionStyle: "bg-primary text-slate-100 hover:bg-primary-dark hover:text-white",
        actionLink: "/register",
    },
    ...
    },
    {
        ...
        details: [
            { icon: "public", text: "Target: Global Markets", color: "text-slate-400" },
            { icon: "update", text: "Updated: Sep 2024", color: "text-slate-400" },
        ],
        action: "Download Guidelines", actionStyle: "border border-white/10 text-slate-300 hover:bg-[#0a0a0a]",
    },
    {
        type: "Funding", category: "Funding & Grants",
        typeColor: "bg-green-50 text-green-700 border-green-200",
        ...
        details: [
            ...
            { icon: "account_balance_wallet", text: "Grant: Up to ₹20 Lakhs", color: "text-slate-400" },
    ...
    },
    {
        ...
        details: [
            { icon: "business", text: "Agency: State Drug Controller", color: "text-slate-400" },
    ...
    },
    {
        ...
        details: [
            { icon: "location_on", text: "Location: AIIA, New Delhi", color: "text-slate-400" },
            { icon: "groups", text: "Rolling Admissions", color: "text-slate-400" },
        ],
        action: "Apply Now →", actionStyle: "bg-primary text-slate-100 hover:bg-primary-dark hover:text-white",
        actionLink: "/register",
    },
    ...
    return (
        <div className="min-h-screen bg-background-light font-display">
            ...
            <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-white/10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16">
                    <Link href="/" className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-lg text-slate-100">AYUSH<span className="font-normal">Hub</span></span>
                        <span className="text-[10px] text-slate-400 uppercase ml-1">Start-up Portal</span>
                    </Link>
                    <div className="hidden md:flex items-center space-x-6">
                        <Link href="/dashboard" className="text-sm text-slate-400 hover:text-slate-100">Dashboard</Link>
                        <Link href="/schemes" className="text-sm font-semibold text-primary-dark border-b-2 border-primary pb-0.5">Schemes & Grants</Link>
                        <Link href="/investors" className="text-sm text-slate-400 hover:text-slate-100">Investors</Link>
                    </div>
                    <div className="flex items-center gap-3">
                        <Link href="/login" className="px-4 py-1.5 bg-primary rounded-full text-sm font-semibold text-slate-100 hover:bg-primary-dark hover:text-white transition-all">Login</Link>
                    </div>
                </div>
            </nav>
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <h1 className="text-4xl md:text-5xl font-bold text-slate-100 mt-4 mb-6">
                ...
                <div className="max-w-2xl mx-auto flex items-center bg-[#111111] rounded-xl border border-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.5)] overflow-hidden px-4">
                    ...
                    />
                    {searchQuery && (
                        <button onClick={() => setSearchQuery("")} className="text-slate-400 hover:text-slate-400 mr-2">
                ...
                <div className="mt-3 text-xs text-slate-400 space-x-3">
                    ...
                    {["Startup India Grant", "Clinical Trials", "GMP Certification"].map((t) => (
                        <button key={t} onClick={() => setSearchQuery(t)} className="text-slate-400 hover:text-primary-dark transition-colors">{t}</button>
                    ))}
                </div>
            </div>
            </section>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${activeCategory === cat
                                    ? "bg-primary text-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
                                    : "bg-[#111111] text-slate-400 border border-white/10 hover:bg-[#0a0a0a]"
                                    }`}
                            >
                                {cat}
                            </button>
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-400">
                        ...
                        <select
                            ...
                            className="border border-white/10 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                        >
                ...
                {filtered.length === 0 && (
                    <div className="text-center py-16">
                        ...
                        <h3 className="text-lg font-bold text-slate-100 mb-2">No resources found</h3>
                        <p className="text-sm text-slate-400 mb-4">Try adjusting your search or filter criteria.</p>
                        <button onClick={() => { setSearchQuery(""); setActiveCategory("All Resources"); }} className="px-4 py-2 bg-primary rounded-lg text-sm font-medium text-slate-100 hover:bg-primary-dark hover:text-white">
                            Clear Filters
                        </button>
                    </div>
                )}
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    {filtered.map((resource, index) => {
                        ...
                        return (
                            <div key={index} className={`bg-white rounded-xl border border-white/5 border-l-4 ${resource.borderColor} overflow-hidden hover:shadow-[0_15px_40px_rgba(0,0,0,0.8)] shadow-black transition-all duration-200 hover:-translate-y-0.5`}>
                                <div className="p-6">
                                    ...
                                    <h3 className="font-bold text-slate-100 mb-2">{resource.title}</h3>
                                    <p className="text-xs text-slate-400 mb-4 leading-relaxed">{resource.desc}</p>

                                    {resource.tags && (
                                        <div className="flex flex-wrap gap-1.5 mb-4">
                                            {resource.tags.map((tag) => (
                                                <span key={tag} className="px-2 py-0.5 bg-[#141414] text-slate-400 text-[10px] rounded-md">{tag}</span>
                                            ))}
                                        </div>
                                    )}
                                        ...
                                        <div className="space-y-2 mb-4">
                                            {resource.details.map((detail, i) => (
                                                <div key={i} className="flex items-center gap-2 text-xs">
                                                    ...
                                                    <span className="text-slate-400">{detail.text}</span>
                                                </div>
                                            ))}
                                        </div>
                                    )}
            ...
            <div className="bg-primary/5 border-y border-primary/10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                        <div><div className="text-3xl font-bold text-slate-100">500+</div><div className="text-sm text-slate-400 mt-1 uppercase tracking-wider">Registered Startups</div></div>
                        <div><div className="text-3xl font-bold text-slate-100">₹200Cr</div><div className="text-sm text-slate-400 mt-1 uppercase tracking-wider">Grants Disbursed</div></div>
                        <div><div className="text-3xl font-bold text-slate-100">1.2k</div><div className="text-sm text-slate-400 mt-1 uppercase tracking-wider">Research Papers</div></div>
                        <div><div className="text-3xl font-bold text-slate-100">50+</div><div className="text-sm text-slate-400 mt-1 uppercase tracking-wider">Incubators</div></div>
                    </div>
                </div>
            </div>
            ...
            <footer className="bg-[#111111] border-t border-white/10 py-12">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
                        <div>
                            <div className="flex items-center gap-2 mb-4">
                                <div className="w-7 h-7 bg-primary rounded-lg flex items-center justify-center text-slate-100 font-bold text-xs">A</div>
                                <span className="font-bold text-slate-100">AYUSH<span className="font-normal">Hub</span></span>
                            </div>
                            <p className="text-xs text-slate-400 leading-relaxed">A Government of India initiative to promote startups and innovation in the field of Ayurveda, Yoga, Unani, Siddha, and Homeopathy.</p>
                        </div>
                        <div>
                            <h4 className="font-bold text-sm text-slate-100 mb-4">Quick Links</h4>
                        ...
                        <div>
                            <h4 className="font-bold text-sm text-slate-100 mb-4">Resources</h4>
                            <ul className="space-y-2 text-xs text-slate-400">
                        ...
                        <div>
                            <h4 className="font-bold text-sm text-slate-100 mb-4">Contact</h4>
                            <ul className="space-y-2 text-xs text-slate-400">
                    ...
                    <div className="border-t border-white/5 flex justify-center py-6 text-sm text-gray-500 font-medium">
