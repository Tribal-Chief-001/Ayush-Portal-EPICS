            ...
            <header className="bg-[#111111] border-b border-white/10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16">
                    <Link href="/" className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-lg text-slate-100">AYUSH <span className="text-primary-dark">Dashboard</span></span>
                    </Link>
                    <div className="flex items-center gap-4">
                        <Link href="/dashboard/support" className="p-2 hover:bg-[#0a0a0a] rounded-lg relative" title="AI Support">
                            <span className="material-icons text-slate-400">smart_toy</span>
                        </Link>
                        <button className="p-2 hover:bg-[#0a0a0a] rounded-lg relative">
                    ...
                    <aside className="w-56 hidden lg:block">
                        <nav className="sticky top-24 space-y-1">
                            ...
                            <div className="pt-4 border-t border-white/10 mt-4">
                                <Link href="/dashboard/support" className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-slate-400 hover:bg-[#0a0a0a]">
                                    <span className="material-icons text-lg">support_agent</span> AI Support
                                </Link>
                                <Link href="/schemes" className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-slate-400 hover:bg-[#0a0a0a]">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                            {[
                                ...
                                <div key={stat.label} className="bg-[#111111] rounded-xl border border-white/5 p-4 hover:shadow-[0_4px_20px_rgba(0,0,0,0.5)] transition-shadow">
                                    ...
                                    <p className="text-sm font-bold text-slate-100 mt-0.5">{stat.value}</p>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="grid lg:grid-cols-3 gap-6">
                            {/* Timeline */}
                            <div className="lg:col-span-2 bg-[#111111] rounded-xl border border-white/5 p-6">
                                <h2 className="font-bold text-slate-100 mb-6 flex items-center gap-2">
                                    <span className="material-icons text-primary-dark">timeline</span> Application Timeline
                                </h2>
                                {!application ? (
                                    <p className="text-sm text-slate-400">No application timeline to display.</p>
                                ) : (
                                                ...
                                                <div className="flex flex-col items-center">
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step.status === "done"
                                                        ...
                                                        : step.status === "current"
                                                            ...
                                                            : "border-white/10 text-slate-300 bg-[#111111]"
                                                        }`}>
                                                        {step.status === "done" ? (
                                                    ...
                                                    {i < timeline.length - 1 && (
                                                        <div className={`w-0.5 h-8 mt-1 ${step.status === "done" ? "bg-primary" : "bg-slate-800"}`}></div>
                                                    )}
                                                </div>
                                                <div className="pt-1">
                                                    <p className={`text-sm font-semibold ${step.status === "pending" ? "text-slate-400" : "text-slate-100"}`}>{step.label}</p>
                            ...
                            <div className="bg-[#111111] rounded-xl border border-white/5 p-6">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    {notifications?.length > 0 && (
                                        <button onClick={() => setData({ ...data, notifications: [] })} className="text-xs text-slate-400 hover:text-slate-400">Clear all</button>
                                    )}
                                </div>
                                {notifications?.length === 0 ? (
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {notifications?.map((n: any) => (
                                            <div key={n.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-[#0a0a0a] transition-colors group">
                                                ...
                                                <button onClick={() => dismissNotification(n.id)} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-slate-400 transition-opacity">
                        ...
                        <div className="mt-6 grid sm:grid-cols-3 gap-4">
                            {[
                                ...
                                <Link key={action.label} href={action.link} className="bg-[#111111] rounded-xl border border-white/5 p-5 hover:shadow-[0_8px_30px_rgba(0,0,0,0.6)] transition-all hover:-translate-y-0.5 group">
                                    ...
                                    <h3 className="font-bold text-sm text-slate-100 group-hover:text-primary-dark">{action.label}</h3>
